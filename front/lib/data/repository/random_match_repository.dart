...
class RandomMatchRepository {
  ...
  Future<void> connectAndSubscribe({
    ...
    try {
      debugPrint(
        '🔌 [RandomMatchRepo] WebSocket 연결 상태 확인: ${wsClient.isConnected}',
      );
  ...
  Future<void> startMatching({required String genderOption}) async {
    debugPrint('🎯 [RandomMatchRepo] 매칭 시작 - genderOption: $genderOption');
  ...
  // 4. 매칭 구독 해제
  void unsubscribeFromMatchUpdates(int userIdx) {
